...
def copy_exact_source(src: Path, repo: Path) -> None:
    ...
    (dest / "PROVENANCE.md").write_text(
        ...
        "This directory preserves the exported source exactly. Do not silently treat it as newer than current production logic. Production fixes, form handling, security, accessibility, and locked brand assets remain authoritative when they conflict.\n"
    )
...
def build_preview(src: Path, repo: Path) -> None:
    ...
    out = require(src / "Malaysia Experience v2.dc.html").read_text()
    ...
    detector = (
        ...
        '<input name="email"><input name="stage"><input name="field"><input name="contact_consent">\n'
        '</form>\n'
    )
    ...
    replacement = r'''
      signUp: () => {
        ...
        fetch('/', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: payload.toString() })
          .then((res) => { if (!res.ok) throw new Error('Signup failed'); try { localStorage.setItem('lb_student_name', nm); localStorage.setItem('lb_lang', this.state.lang); } catch (_) {} this.setState({ joined: true, seatErr: '' }); })
    ...
    (landing / "claude-design-preview.html").write_text(out)


def write_status(repo: Path) -> None:
    (repo / "CLAUDE_DESIGN_SYNC_STATUS.md").write_text(
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("Usage: import-claude-design-export.py <extracted-export-dir> <repo-root>")
